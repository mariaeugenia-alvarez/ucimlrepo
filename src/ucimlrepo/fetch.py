...
def fetch_ucirepo(name: Optional[str] = None, id: Optional[int] = None):
    """
    ...
    try:
        response = urllib.request.urlopen(
            api_url, context=ssl.create_default_context(cafile=certifi.where())
        )
    ...
    if data["status"] != 200:
        error_msg = (
            data["message"] if "message" in data else "Dataset not found in repository"
        )
    ...
    try:
        ...
        # Check if running on macOS and apply SSL fix
        if platform.system() == "Darwin":  # Darwin is the system name for macOS
            try:
                # Create non verified SSL context for Mac
                ...
                # Try reading CSV again with disabled SSL verification
                df = pd.read_csv(data_url)

            except Exception as ssl_e:
                raise DatasetNotFoundError(
                    'Error reading data csv file for "{}" dataset (id={}) on macOS. SSL verification disabled but still failed: {}'.format(
                        name, id, str(ssl_e)
                    )
                )
        else:
            raise DatasetNotFoundError(
                'Error reading data csv file for "{}" dataset (id={}).'.format(name, id)
            )
...
def list_available_datasets(
    ...
    # print table title
    title = "The following {}datasets are available{}:".format(
        filter + " " if filter else "",
        ' for search query "{}"'.format(search) if search else "",
    )
    print("-" * len(title))
    print(title)
    if area:
    ...
    for dataset in data:
        row_str = "{:<{width}} {:<6}".format(
            dataset["name"], dataset["id"], width=maxNameLen
        )
